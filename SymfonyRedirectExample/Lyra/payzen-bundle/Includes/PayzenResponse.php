...
if (! class_exists('PayzenResponse', false)) {
    ...
    class PayzenResponse
    {
        ...
        public function isAcceptedPayment()
        {
         ...
         /**
         * Check if the payment was with any value in pageAction
         * @return bool
         */
        public function verifyValueInPageAction(string $value)
        {
            $page_action = $this->rawResponse["vads_page_action"];
            if($page_action == $value){
                return true;
            }else{
                return false;
            }
        ...
        public function getOutputForGateway($case = '', $extra_message = '', $original_encoding = 'UTF-8')
        {
            ...
            $cases = array(
                ...
                'register_ok' => array(true, 'Accepted verification card.'),
                'payment_ko' => array(true, 'Payment failure, order has been cancelled.'),
                'register_ko' => array(true, 'Refused verification card'),
